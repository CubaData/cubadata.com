...
export function ArticlePage({ slug, onNavigate }: ArticlePageProps) {
  ...
  const handleShare = (platform: 'main' | 'facebook' | 'twitter' | 'pinterest' | 'whatsapp') => {
    ...
    switch (platform) {
      ...
      case 'twitter':
        ...
        break
      case 'pinterest':
        const media = article.image ? (article.image.startsWith('http') ? article.image : window.location.origin + article.image) : ''
  ...
  return (
    <main className="post-page article-page">
      <div className="container">
        ...
        <div className="two-column">
          <article className="single-post article-post">
            ...
            {article.link && (
              <AppLink className="pdf-download-button" href={article.link} target="_blank" onNavigate={onNavigate}>
                <DownloadIcon />
